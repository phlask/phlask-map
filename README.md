...
└── src                                <-- Source files for project
    ...
    ├── contexts
    ├── hooks                          <-- Custom hooks
    ├── icons                          <-- Autogenerated Svg icon components (from `assets/icons`)
    ├── services                       <-- 3rd party clients we utilize (db, routing, query client)
    ├── schemas                        <-- Schemas for form validations (powered by Zod)
    ├── types                          <-- Typescript type definitions
    ├── utils                          <-- Typescript type definitions
    └── theme.ts                       <-- Theme file for Material UI
```
...
1. Ensure you have [nodejs](https://nodejs.org/en/download) installed on your machine.
   - You can also use [nvm](https://github.com/nvm-sh/nvm/tree/master#install--update-script).
   - once you have install nvm
   - run `nvm install` <- this will download the required version (only required on the first installation)
   - run `nvm use` sets that to the active version of node in your terminal
2. Ensure you have [pnpm](https://pnpm.io/) installed on your machine
3. Run `pnpm install`
4. Run `pnpm start`
...
We use Prettier to ensure a consistent code formatting across the project, if you are running VSCode/Cursor, please make sure to install the [Prettier extension](https://marketplace.visualstudio.com/items?itemName=esbenp.prettier-vscode), the project configuration will ensure that Prettier is set as your default formatter as well.

### Code Linter

We use ESLint to enforce consistent coding guidelines. Please ensure to download the ESLint extension

### Typescript

We use Typescript for static type checking of our JavaScript code. It offers helpful tools for consistent and safe coding.
