...
export const updateResource = async (resource) => {
    const { data, error } = await supabase.from(resourceDatabaseName).upsert(resource);
    if (error) {
        throw error;
    }
    return data;
...
export const addResource = async (resource) => {
    const { data, error } = await supabase.from(resourceDatabaseName).insert(resource);
    if (error) {
        throw error;
    }
    return data;
 ...
 * @returns {Promise<Array>} - An array of contributors
 */
